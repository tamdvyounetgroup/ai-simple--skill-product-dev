---
description: Tự audit project theo 15 nguyên tắc ai-simple-product-dev → điểm theo applicability + backlog tối ưu xếp hạng (chạy mỗi quý hoặc khi nghi hệ docs đang mục)
...
| 14 | Security gate | hook có `SEC_CHECKS=auto` chưa (grep `.githooks/pre-commit`); fixture SEC trong `--self-test` PASS? security-review (nếu có) đủ `declared-coverage:` + mã OWASP + contract "pentest" (`security-verify.sh` exit 0)? CI có step SCA? Path nhạy cảm (auth/payment/migrations) từng commit KHÔNG kèm review → đếm WARN bỏ qua. KHÔNG áp dụng khi project không có code chạm secret/auth/dep ngoài → NOT_APPLICABLE. Tìm "đã bảo mật ✓"/over-claim trong docs → mỗi cái là finding (14 cấm tick trần) |
| 15 | Build discipline | 3 số nợ: `grep -rnoE '(//\|#\|--) ?(nợ\|debt):' src \| wc -l` (tổng) · số THIẾU vế nâng cấp (grep -vE 'nợ:[^,]+,' — số đáng lo) · export trùng tên còn lại (loại route handler). Hook có `BUILD_CHECKS` + mục 1f chưa? Ở repo nguồn ai-simple: `npm run rule-enforcer` PASS + tỷ lệ luật chỉ-người-ép ≤ 40%; so 3 commit trong `NOTICE.md` với HEAD 3 nguồn thượng nguồn → ra ĐÚNG 1 dòng "có/không ý tưởng mới đáng nội hoá", không diff chi tiết |
