...
update_marker() {
  # v1.19.0 (audit ForFish, MINOR-4): KHÔNG ghi khi nội dung marker không đổi — trước đây `sed -i`
  # chạy vô điều kiện trên MỌI doc nên mỗi lần chạy làm bẩn `git status` cả 19 doc (và trên Windows
  # còn đổi EOL). Chỉ đụng file khi marker thật sự khác.
  WANT=""
  if [ "$2" = "SUSPECT" ] || [ "$2" = "ORPHANED" ]; then
    WANT="<!-- DOC-STATUS: $2 ($(date +%Y-%m-%d)) — $3. DOI CHIEU VOI CODE truoc khi tin. May quan ly dong nay, dung sua tay. -->"
  fi
  HAVE=$(grep -m1 '<!-- DOC-STATUS:' "$1" 2>/dev/null || true)
  [ "$HAVE" = "$WANT" ] && return 0
...
EOF_DOCS1
  ...
  while IFS="$(printf '\t')" read -r doc DS; do
    [ -n "$doc" ] || continue
    ST=$(echo "$DS" | cut -d'|' -f1)
